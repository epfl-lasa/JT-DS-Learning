...
\resizebox{\linewidth}{!}{\begin{tabular}{cc|cc|cc}
   ...
   \multicolumn{1}{c}{\multirow{3}{*}{($M=11,M^*T=1232$)}} & PCA & 3.8 $\pm$ (0.421) & 3.8 $\pm$ (1.549) & 0.292 $\pm$ (0.018) & \cellcolor{blue!10} 0.486 $\pm$ (0.125) \\	
	...
	\multicolumn{1}{c}{\multirow{3}{*}{\textcolor{red}{Pouring - Obstacle 2}}} & None & 7 & X $\pm$ (Y) & X $\pm$ (Y) & X $\pm$ (Y) \\
   \multicolumn{1}{c}{\multirow{3}{*}{($M=X,M^*T=Y$)}} & PCA & X $\pm$ (Y) & X $\pm$ (Y) & X $\pm$ (Y) & X $\pm$ (Y) \\	
				& K-PCA ($\sigma = X$) & X $\pm$ (Y) & X $\pm$ (Y) & X $\pm$ (Y) & X $\pm$ (Y)  \\
				\hline
...
\noindent For each behavior, we collected $N$ demonstrations consisting of ($14$-D) time-series $D=\{\{ q_{t,n},\dot{q}_{t,n} \}_{t=1,\dots, T_n}\}_{n=1,\dots,N}$ where each demonstration has duration $T_n$. We also record the final task-space configurations ${x^*}^{(n)}$, which we assume is the attractor in task-space. The recordings are collected at a rate of 500 Hz. However, our learning approach does not require such a dense representation of the trajectories, hence we down-sample them to 25 Hz. The total number of demonstrations $N$ and samples per \textit{dataset} $M = \sum_{n=1}^{N}T_n$ are provided in Table \ref{tab:learning}. Given these datasets, we seek to: 
\begin{enumerate}[leftmargin=*]
\item Verify that the learned \textit{behavior synergies} are capable of generating the demonstrated joint-space behaviors.  
...
%		\centering
%			\includegraphics[width=0.5\linewidth]{new-pics/forehand_crop.png}\hspace{5pt}\includegraphics[width=0.43\linewidth]{new-pics/backhand_crop.png}
%					\caption{Tennis Strokes (left) Forehand (right) Backhand.}
%		\end{subfigure}
%		\begin{subfigure}[t]{\linewidth}
%\centering
%	\includegraphics[width=0.45\linewidth]{new-pics/pour-free_crop.png}\hspace{5pt}\includegraphics[width=0.477\linewidth]{new-pics/pour-obst_crop.png}
%						\caption{Pouring Motions (left) without obstacle (right) with obstacle 1.}
%			\end{subfigure}
%			\begin{subfigure}[t]{\linewidth}		
%\centering
%	\includegraphics[width=0.45\linewidth]{new-pics/pour-obst-2_crop.png}\hspace{5pt}\includegraphics[width=0.5\linewidth]{new-pics/footstep.png}
%	\caption{(left) Pouring Motions with obstacle (right) with obstacle 2 (right) Foot-step motion.}
%				\end{subfigure}
%	\caption{Demonstrated Joint-Space Behaviors with Task-Space Targets. \href{https://www.dropbox.com/sh/3vx512oldgbnkq5/AABP5iEqO0oYn7dlE-gvK1kPa?dl=0}{[videos]} \label{fig:behaviors}}
%	\vspace{-10pt}
%\end{figure}
%
%\begin{table*}[!t]
...
%	& K-PCA ($\sigma = 1.769$) & 3.6 $\pm$ (0.516) & \textbf{5.3 $\pm$ (2.452)} & 0.055 $\pm$ (0.018) & \cellcolor{blue!15} 0.071 $\pm$ (0.010)   \\ \hline\hline
%\end{tabular}
%\caption{\scriptsize Performance Comparison of Learning Approach with different Dimensionality Reduction Schemes. We present the \texttt{mean(std)} for the optimal lower-dimensional $d$ and behavior synergies $K$ and RMSE on training and testing set, found for every learning scheme over 10 runs.\label{tab:learning}}
%\end{center}
%\vspace{-10pt}
%\end{table*}
...
We now evaluate the performance of JT-DS\footnote{With the optimal learning approach found in the previous section.} against a standard Cartesian motion generator + IK solver approach. In this evaluation we seek to compare the capabilities (or lack thereof) of both approaches to: 
\begin{enumerate}
...
%Human experts guided the robot through a series of demonstrations, ensuring that neither the robot's joints nor the held object intersected the obstacle. 
...
\begin{table}[t]
	...
	\resizebox{\linewidth}{!}{\begin{tabular}{l|c|c|c|c}
		...
		Obstacle avoidance (JT-DS)  &      7                &     4             &        4  &  0.0137    \\ 
		Obstacle avoidance (SEDS) &      7                &     3             &        -  &   -   \\		
		\hline
		Foot step  (JT-DS) &       3               &     1             &          1 & 0.01344   \\ 
		Foot step (SEDS)&       3               &     1             &          - & -   \\ \hline \hline
	\end{tabular}}
