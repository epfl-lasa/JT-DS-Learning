...
%     end
    ...
    switch type
        ...
        case {'AutoEncoder', 'Autoencoder'}
            ...
            layers = [ceil(size(A, 2)*1.2) + 5, max([ceil(size(A, 2) / 4), no_dims + 2]), no_dims];
...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
            ...
            else [network, mappedA] = train_deep_autoenc(A, layers, varargin{1}, varargin{2}); end
        ...
        case 'None'
            ...
            mappedA = A;
            mapping.M = eye(size(A, 2));
            mapping.mean = zeros(1, size(A, 2));
            mapping.name = 'None';
