...
%     if no_dims < 1 % if so, no_dims is fraction of desired explained variance
%         target_explained_variance = no_dims;
%         explained_variance = L./(sum(L));
%         no_dims = sum(cumsum(explained_variance) < target_explained_variance)+1;
%     end
%     if no_dims > size(M, 2)
%         no_dims = size(M, 2);
