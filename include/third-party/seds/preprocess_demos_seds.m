function [Data] = preprocess_demos_seds(robotplant, demos,orientation_flag)
...
for i=1:length(demos)
    ...
    if orientation_flag
        xd=zeros(6,size(demos{i},2));
        for j=1:size(demos{i},2)
            trans_tmp=robotplant.robot.fkine(demos{i}(:,j));
            axang = rotm2axang([trans_tmp(1:3,1) trans_tmp(1:3,2) trans_tmp(1:3,3)]);
            xd(:,j) = [transpose(axang(1:3)*axang(4));trans_tmp(1:3,end)];
        end
    else
        xd=zeros(3,size(demos{i},2));
        for j=1:size(demos{i},2)
            trans_tmp=robotplant.robot.fkine(demos{i}(:,j));
            xd(:,j) = trans_tmp(1:3,end);
        end
    end
